...
for i in range(data_index.shape[0]):
    if (name != data['Name'][i+1]):
        ...
        if data['Prop15'].iloc[i] > 0:
            ...
            data['Prop00'].iloc[i] = np.nan

#data.apply(lambda x: x['Grade']=='REF1', axis=1)
#data5 = data[data['Grade']=='REF5']
#data[['Name','Prop15','Prop17','Prop_New']][0:20]
i = data[data['Prop00'].isnull() == True].index
data_drop = data.drop(i)

data_drop.shape
sns.set(style="darkgrid")
sns.countplot(x="Prop00", hue="Grade", data=data_drop)
sns.catplot(x='Name',y='Prop15',hue='Grade',data=data)
sns.catplot(x='Name',y='Prop17',hue='Grade',data=data)
